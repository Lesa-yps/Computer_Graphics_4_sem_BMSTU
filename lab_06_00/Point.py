...
def draw_line(cnv: tk.Canvas, xs: int, ys: int, edges_arr: List[Tuple[int]], color: str = COLOR_LINE) -> None:
    if len(edges_arr) == 0:
        ...
        # print(point1, point2, "pixels = ", pixels)
...
def touch(x_input: int, y_input: int, cnv: tk.Canvas, tree: ttk.Treeview, ZOOM: int, SIDE_PLACE: int, HEIGHT_PLACE: int,
          edges_arr: List[Tuple[int]], is_painting: bool, change_coord: bool = True) -> None:
    if is_painting:
...
def check_input_field(x_entry: tk.Entry, y_entry: tk.Entry, help_str: str) -> bool:
    try:
        int(x_entry.get())
        int(y_entry.get())
    except ValueError:
        mb.showerror(
            'Ошибка!', f"Оба поля координат должны быть заполнены ({help_str}).")
        return False
    else:
        return True
