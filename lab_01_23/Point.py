...
def point_in_table(arr: List[Tuple[int, int]], point: Tuple[int, int]) -> bool:
    for i in arr:
...
def touch(x_input: int, y_input: int, cnv: tk.Canvas, tree: ttk.Treeview, ZOOM: float, SIDE_PLACE: int, HEIGHT_PLACE: int,
    ...
    if point_in_table(arr, (x_table, y_table)) and check_in_table:
        ...
        weight = 4 * ZOOM
        cnv.create_oval(x_input - weight, y_input - weight, x_input + weight,
                        y_input + weight, fill="red", outline="red", tags="point")
        cnv.create_oval(x_input - ZOOM, y_input - ZOOM, x_input + ZOOM,
                        y_input + ZOOM, fill="black", outline="black", tags="point")
...
def draw_points_from_table(cnv: tk.Canvas, tree: ttk.Treeview, ZOOM: float, SIDE_PLACE: int, HEIGHT_PLACE: int) -> None:
    ...
    for item in items:
        ...
        touch(x_table, y_table, cnv, tree, ZOOM, SIDE_PLACE,
              HEIGHT_PLACE, change_coord=False, check_in_table=False)
...
def del_point(cnv: tk.Canvas, tree: ttk.Treeview, ZOOM: float, SIDE_PLACE: int, HEIGHT_PLACE: int) -> None:
    ...
    if selected_item:
        ...
        cnv.delete("point")
        # отрисовывает все точки из таблицы
        draw_points_from_table(cnv, tree, ZOOM, SIDE_PLACE, HEIGHT_PLACE)
    else:
