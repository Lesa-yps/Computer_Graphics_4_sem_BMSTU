...
def fork(text: str, x_entry: tk.Entry, y_entry: tk.Entry, cnv: tk.Canvas, tree: ttk.Treeview) -> None:
    ...
    if text == 'Добавить' and check_input_field(x_entry, y_entry):
        touch(int(x_entry.get()), int(y_entry.get()), cnv,
              tree, ZOOM, SIDE_PLACE, HEIGHT_PLACE, False)
        x1.delete(0, "end")
        y1.delete(0, "end")
    # Удалить точку
    elif text == 'Удалить':
...
make_button('Редактировать', button_frame1, 13).grid(
...
make_button('Построить результат', button_frame2,
            20).grid(row=0, column=0, stick='we')
make_button('Очистить холст', button_frame2, 25).grid(
...
cnv.bind('<Button-1>', lambda event: touch(event.x, event.y,
         cnv, tree, ZOOM, SIDE_PLACE, HEIGHT_PLACE))
...
menu_in.add_command(label='Добавить точку',
                    command=lambda: fork('Добавить', x1, y1, cnv, tree))
menu_in.add_command(label='Удалить точку',
                    command=lambda: fork('Удалить', x1, y1, cnv, tree))
menu_in.add_command(label='Редактировать точку',
                    command=lambda: fork('Редактировать', x1, y1, cnv, tree))
menu_in.add_command(label='Построить результат', command=lambda: fork(
    'Построить результат', x1, y1, cnv, tree))
menu_in.add_command(label='Очистить холст', command=lambda: fork(
    'Очистить холст', x1, y1, cnv, tree))
