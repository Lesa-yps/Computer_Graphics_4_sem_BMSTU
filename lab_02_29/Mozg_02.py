...
def for_each_turple(arr_turple: List[Tuple[int, int]], func_x, param_x, func_y, param_y) -> List[Tuple[int, int]]:
    #print(arr_turple)
...
def func_move_y(elem: Tuple[int, int], ds: int) -> int:
    return elem[1] + ds
# перенос
def brain_move_house(dx: int, dy: int, dict_house: Dict[str, Any]) -> Dict[str, Any]:
...
def func_scale_y(elem: Tuple[int, int], param: Tuple[float, float]) -> float:
    ...
    return y1
# масштабирование
def brain_scale_house(kx: float, ky: float, x_m: float, y_m: float, dict_house: Dict[str, Any]) -> Dict[str, Any]:
...
def func_turn_y(elem: Tuple[int, int], param: Tuple[float, float, float]) -> float:
    ...
    return y1
# поворот
def brain_turn_house(angle: float, x_t: int, y_t: int, dict_house: Dict[str, Any]) -> Dict[str, Any]:
    dx, dy = dict_house["center"]
    dict_house = brain_move_house(-dx, -dy, dict_house)
