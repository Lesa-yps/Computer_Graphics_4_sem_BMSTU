...
def for_each_elem_house(dict_house: Dict[str, Any], func_x, param_x, func_y, param_y) -> Dict[str, Any]:
    dict_house["center"] = (func_x(dict_house["center"], param_x), func_y(
        dict_house["center"], param_y))
    dict_house["grass"] = for_each_tuple(
        dict_house["grass"], func_x, param_x, func_y, param_y)
    dict_house["rect"] = for_each_tuple(
        dict_house["rect"], func_x, param_x, func_y, param_y)
    dict_house["roof"] = for_each_tuple(
        dict_house["roof"], func_x, param_x, func_y, param_y)
    dict_house["wind"]["center"] = (func_x(dict_house["wind"]["center"], param_x), func_y(
        dict_house["wind"]["center"], param_y))
    dict_house["wind"]["a"] = (func_x(dict_house["wind"]["a"], param_x), func_y(
        dict_house["wind"]["a"], param_y))
    dict_house["wind"]["b"] = (func_x(dict_house["wind"]["b"], param_x), func_y(
        dict_house["wind"]["b"], param_y))
    dict_house["wind_frame"] = for_each_tuple(
        dict_house["wind_frame"], func_x, param_x, func_y, param_y)
    dict_house["door"] = for_each_tuple(
        dict_house["door"], func_x, param_x, func_y, param_y)
    dict_house["door_handle"] = for_each_tuple(
        dict_house["door_handle"], func_x, param_x, func_y, param_y)
    return dict_house
...
def for_each_tuple(arr_tuple: List[Tuple[int, int]], func_x, param_x, func_y, param_y) -> List[Tuple[int, int]]:
    for i in range(len(arr_tuple)):
        arr_tuple[i] = (round(func_x(arr_tuple[i], param_x)),
                        round(func_y(arr_tuple[i], param_y)))
    return arr_tuple
...
def brain_move_house(param: Tuple[int, int], dict_house: Dict[str, Any]) -> Dict[str, Any]:
    ...
    dict_house = for_each_elem_house(
        dict_house, func_move_x, dx, func_move_y, dy)
...
def brain_scale_house(param: Tuple[float, float, int, int], dict_house: Dict[str, Any]) -> Dict[str, Any]:
    ...
    param_x, param_y = (kx, x_m - dx), (ky, y_m - dy)
    dict_house = for_each_elem_house(
        dict_house, func_scale_x, param_x, func_scale_y, param_y)
    dict_house = brain_move_house((dx, dy), dict_house)
    return dict_house
...
def func_turn_x(elem: Tuple[int, int], param: Tuple[float, float, float]) -> float:
    x1, y1 = elem
    x_c, y_c, angle = param
    x1 = x_c + (x1 - x_c) * math.cos(math.radians(angle)) + \
        (y1 - y_c) * math.sin(math.radians(angle))
    return x1
...
def func_turn_y(elem: Tuple[int, int], param: Tuple[float, float, float]) -> float:
    x1, y1 = elem
    x_c, y_c, angle = param
    y1 = y_c - (x1 - x_c) * math.sin(math.radians(angle)) + \
        (y1 - y_c) * math.cos(math.radians(angle))
    return y1
# поворот
...
def brain_turn_house(param: Tuple[float, int, int], dict_house: Dict[str, Any]) -> Dict[str, Any]:
    ...
    dict_house = for_each_elem_house(
        dict_house, func_turn_x, param_func, func_turn_y, param_func)
    dict_house = brain_move_house((dx, dy), dict_house)
    return dict_house

# x(α)=a*cos(α)cos(θ)−b*sin(α)sin(θ)+Cx
# y(α)=a*cos(α)sin(θ)+b*sin(α)cos(θ)+Cy
# α - это параметр, который находится в диапазоне от 0 до 2π радиан
# θ - это угол поворота эллипса
