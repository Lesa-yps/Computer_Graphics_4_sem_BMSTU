...
from House import draw_house, copy_house, constructor_house
...
def fork(text: str, dx: tk.Entry, dy: tk.Entry, kx: tk.Entry, ky: tk.Entry, x_m: tk.Entry, y_m: tk.Entry,
    ...
    if text == 'Перенести' and check_input_field([dx, dy], "Оба параметра переноса должны быть заполнены."):
        copy_change_draw_house(
            brain_move_house, (float(dx.get()), float(dy.get())))
    # масштабирование
        ...
        copy_change_draw_house(brain_scale_house, (float(kx.get()), float(
            ky.get()), round(float(x_m.get())), round(float(y_m.get()))))
    ...
    # поворот
        ...
        copy_change_draw_house(brain_turn_house, (float(
            angle_turn.get()), round(float(x_t.get())), round(float(y_t.get()))))
...
tk.Label(input_frame, text="Коэффициенты масштабирования:", font=(
    "Calibry", 12), bg="light pink").grid(row=0, column=0)
tk.Label(input_frame, text="dx:", font=("Calibry", 12),
         bg="light pink").grid(row=1, column=0)
...
tk.Label(input_frame, text="Координаты центра масштабирования:",
         font=("Calibry", 12), bg="light pink").grid(row=3, column=0)
tk.Label(input_frame, text="X:", font=("Calibry", 12),
         bg="light pink").grid(row=4, column=0)
x_m = tk.Entry(input_frame, font=("Calibry", 12))
x_m.grid(row=4, column=1)
tk.Label(input_frame, text="Y:", font=("Calibry", 12),
         bg="light pink").grid(row=5, column=0)
...
tk.Label(input_frame, text="Угол поворота:", font=(
    "Calibry", 12), bg="light pink").grid(row=0, column=0)
angle_turn = tk.Entry(input_frame, font=("Calibry", 12))
angle_turn.grid(row=0, column=1)
tk.Label(input_frame, text="Координаты центра поворота:", font=(
...
def make_button(doing: str, button_frame: tk.Frame, width1: int) -> tk.Button:
    return tk.Button(button_frame, text=doing, bd=7, font=("Calibry", 12),
                     command=lambda: fork(
                         doing, dx, dy, kx, ky, x_m, y_m, angle_turn, x_t, y_t, cnv),
...
make_button('Масштабировать', button_frame1, 14).grid(
...
tk.Label(window, text="Талышева Олеся ИУ7-45Б", bg='light pink',
         fg='grey', font=("Arial", 12, 'italic')).grid(row=6, column=0)
...
cnv.bind('<Button-3>', lambda event: fork('Шаг назад', dx,
         dy, kx, ky, x_m, y_m, angle_turn, x_t, y_t, cnv))
...
menu_in.add_command(label='Перенести', command=lambda: fork(
    'Перенести', dx, dy, kx, ky, x_m, y_m, angle_turn, x_t, y_t, cnv))
menu_in.add_command(label='Масштабировать', command=lambda: fork(
    'Масштабировать', dx, dy, kx, ky, x_m, y_m, angle_turn, x_t, y_t, cnv))
menu_in.add_command(label='Повернуть', command=lambda: fork(
    'Повернуть', dx, dy, kx, ky, x_m, y_m, angle_turn, x_t, y_t, cnv))
menu_in.add_command(label='Шаг назад', command=lambda: fork(
    'Шаг назад', dx, dy, kx, ky, x_m, y_m, angle_turn, x_t, y_t, cnv))
menu_in.add_command(label='Сброс', command=lambda: fork(
    'Сброс', dx, dy, kx, ky, x_m, y_m, angle_turn, x_t, y_t, cnv))
...
menu_inf.add_command(label='Информация об авторе', command=lambda: mb.showinfo(
    'Информация об авторе', "автор - Талышева ИУ7-45Б"))
